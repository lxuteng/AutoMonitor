...
class Db:
    ...
    def displaydata(self, datatype='main'):
        ...
        def PHR异常():
            self.kpi_dict = {}
            for n in ini.cf_sql.options('PHR异常'):
                n_child = ini.cf_sql.get('PHR异常', n).split(',')
                if len(n_child) != 2:
                    print('>>> [kpi] 设置异常，请检查！')
                    sys.exit()
                self.kpi_dict[n.lower()] = n_child
...
class Report:
    ...
    def raw_monitor(self, type):
        ...
        # PHR异常小区通报
        if ini.config_raw_monitor_report['active_phr_error_report'] == '1':
            ini.add_report(subject_x='PHR',
                           sql_name='cell_raw_all_phr',
                           sel_self_name='cell_raw_all_phr',
                           timetype_local='top',
                           counter_local='',
                           threshold_local='',
                           datatype_local='PHR异常',
                           report_name='PHR异常',
                           disable_x_type='',
                           dir_name='PHR异常',
                           warning_text='',
