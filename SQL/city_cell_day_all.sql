...
,Round(sum(M8012C19)/1000/1000/1000/1000,2)   用户面PDCP上行数据量TB
,Round(sum(M8012C20)/1000/1000/1000/1000,2)   用户面PDCP下行数据量TB
,Round(sum(M8012C19+M8012C20)/1000/1000/1000/1000,2)   用户面PDCP总数据量TB
...
,case when "QCI1话务量Erl_亿阳" > 1 
			and "QCI1话务量Erl_亿阳" is not null 
			and Decode((M8006C206+M8006C215)+ decode((M8006C54),0,0,ceil((M8006C45)/(M8006C54))),0,0,round(100*
			(M8006C176+M8006C269+M8006C273)/((M8006C206+M8006C215)+ decode((M8006C54),0,0,ceil((M8006C45)/(M8006C54)))),2)) > 1
...
,case when M8016C35 > 5
			and M8016C35 is not null
			and Round(Decode(M8016C33,0,100,100*(M8016C33-M8016C35)/(M8016C33)),2) > 5
			and Round(Decode(M8016C33,0,100,100*(M8016C33-M8016C35)/(M8016C33)),2) is not null
		then 1
		else 0 end srvcc切换差小区数
...
,lncel.co_object_instance cell_id
  ,lnbts.co_object_instance || '_' || lncel.co_object_instance enb_cell     
        ...
        group by
             ...
             ,lnbts.co_object_instance,lncel.co_object_instance,lnbts.co_object_instance || '_' || lncel.co_object_instance
              
)M8012
WHERE      M8012.cel_key_id=m8006.cel_key_id(+)
group by 
substr(M8012.sdate,1,10),enb_id,enb_cell,M8006.LNCEL_ID
)
...
--,enb_cell,enb_id,bts_version,cel_name
ORDER BY --city,
substr(sdate,1,8)--,enb_cell,enb_id
