select ENB_ID,ENB_CELL,BTS_VERSION
,count(ENB_CELL) 最近一个钟出现次数
,round(avg(小区可用率),2) 小区可用率
,sum(QCI1掉线次数) QCI1掉线次数
,sum(QCI1掉线分母小区级)  QCI1掉线分母小区级
,round(avg(QCI1掉线率小区级),2) QCI1掉线率小区级
,round(avg(无线掉线率),2) 无线掉线率
,round(sum(QCI1话务量ERL),2) QCI1话务量ERL
,round(avg(上行丢包率),2) 上行丢包率
,round(avg(下行丢包率),2) 下行丢包率
...
,case when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=8 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=8 then round(decode(avg(M8001C217)+ avg(M8001C216),0,0,(avg(M8011C50)/(24*60*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)/(avg(M8001C217)+ avg(M8001C216)))*100,2) 
      when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=10 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=10 then round(decode(avg(M8001C217)+ avg(M8001C216),0,0,(avg(M8011C50)/(60*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)/(avg(M8001C217)+ avg(M8001C216)))*100,2)
      when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=12 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=12 then round(decode(avg(M8001C217)+ avg(M8001C216),0,0,(avg(M8011C50)/(15*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)/(avg(M8001C217)+ avg(M8001C216)))*100,2)
end 无线利用率  --2017年10月13日更新
,case when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=8 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=8 then round(avg(M8011C50)/(24*60*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)
      when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=10 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=10 then round(avg(M8011C50)/(60*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)
      when length(TO_CHAR(SYSDATE-5/96,'yyyymmddHH24MI'))=12 and length(TO_CHAR(SYSDATE,'yyyymmddHH24MI'))=12 then round(avg(M8011C50)/(15*60*1000*1/5) + avg(M8011C37)/10*avg(M8001C216)/100)
...
,M8006C244,M8006C245,M8006C248,M8006C249,M8006C252,M8006C253
,M8009C15,M8009C16,M8014C23,M8014C24,M8014C26,M8014C27,M8006C287,M8006C291,M8016C54
...
,case when round((M8006C45)/3600,2) > 1 
...
FROM NOKLTE_PS_LCELLD_MNC1_raw PMRAW
        where
         ...
         period_start_time between SYSDATE-5/96 and SYSDATE
...
select
        ...
        from
             NOKLTE_PS_LPQDL_MNC1_raw PMRAW
        where
         period_start_time between SYSDATE-5/96 and SYSDATE
            --- to_char(period_start_time,'yyyymmddHH24MI') >= to_char(SYSDATE-1,'yyyymmddHH24MI')
            -- and to_char(period_start_time,'yyyymmddHH24MI') <= to_char(SYSDATE-1,'yyyymmddHH24MI')
...
,SUM(NVL(INTER_HO_ATT_NB,0))  M8015C8  --The number of Inter eNB Handover attempts per neighbour cell relationship
       ...
       from
             NOKLTE_PS_LNCELHO_DMNC1_RAW PMRAW
        where
        period_start_time between SYSDATE-5/96 and SYSDATE
          ---   to_char(period_start_time,'yyyymmddHH24MI')  >= to_char(SYSDATE-1,'yyyymmddHH24MI')
            -- and to_char(period_start_time,'yyyymmddHH24MI') <= to_char(SYSDATE-1,'yyyymmddHH24MI')
...
,sum(nvl(DENOM_CELL_AVAIL,0)) M8020C6 --The number of samples when cell availability is checked. This counter is used as a denominator for the cell availability calculation
        from
            NOKLTE_PS_LCELAV_MNC1_raw  PMRAW  
        where
             period_start_time between SYSDATE-5/96 and SYSDATE
             --AND PERIOD_DURATION=15
        group by
...
where 
comm.period_start_time between SYSDATE-5/96 and SYSDATE
...
where 小区可用率<=90
and (无线掉线率 is not null
or 上行丢包率 is not null)
)
group by ENB_CELL,ENB_ID,BTS_VERSION
having count(ENB_CELL) >= 3
order by ENB_CELL
