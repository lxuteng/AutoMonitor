...
,Decode(sum(M8006C206+M8006C215)+ decode(sum(M8006C54),0,0,ceil(sum(M8006C45)/sum(M8006C54))),0,0,round(100*Sum(M8006C176+M8006C269+M8006C273)/(sum(M8006C206+M8006C215)+ decode(sum(M8006C54),0,0,ceil(sum(M8006C45)/sum(M8006C54)))),2)) QCI1掉线率
,Sum(M8006C176+M8006C269+M8006C273) QCI1掉线次数
,sum(M8006C206+M8006C215)+ decode(sum(M8006C54),0,0,ceil(sum(M8006C45)/sum(M8006C54))) QCI1掉线分母
...
and kpi.enb_id = pm.enb_id

and erl."QCI1话务量Erl_亿阳" > 0.8
and erl."QCI1话务量Erl_亿阳" < 1
and kpi.QCI1掉线率 > 1
and pm.mtEPSBearer_8006 != 'disable'
--and kpi.enb_cell in ('701350_1')
order by kpi.enb_cell
