...
,sum(M8006C0)-sum(ERAB建立成功率分子) ERAB建立失败数
...
-- ,'{*_*}' 接入类 
 ...
 ,sum(M8006C189+M8006C198)-sum(M8006C207+M8006C216) qci2_erab建立失败次数
-- ,'{*_*}' 切换类 
...
,sum(M8009C15+M8014C23+M8014C26)-sum(M8009C16+M8014C24+M8014C27) VOLTE切换失败次数 --2017年12月15日新增
...
,sum(M8006C273+M8006C266-M8006C267-M8006C270-M8006C272) QCI1掉线次数_监控_1
,sum(M8006C268) QCI1掉线次数_监控_2
FROM
(
SELECT comm.sdate
          --   ,M8020.MRBTS_ID
...
,M8009C15,M8009C16,M8014C23,M8014C24,M8014C26,M8014C27,M8006C287,M8006C291,M8016C54,M8006C266,M8006C267,M8006C270,M8006C272,M8006C268
...
,sum(nvl(ERAB_REL_ENB_QCI1,0)) M8006C266
,sum(nvl(ERAB_REL_ENB_RNL_INA_QCI1,0)) M8006C267
,sum(nvl(ERAB_REL_ENB_RNL_RED_QCI1,0)) M8006C270
,sum(nvl(ERAB_REL_ENB_RNL_RRNA_QCI1,0)) M8006C272
,sum(nvl(ERAB_REL_ENB_RNL_UEL_QCI1,0)) M8006C268
        from
...
where 
comm.period_start_time between to_date(&1,'yyyymmddHH24MI') and to_date(&2,'yyyymmddHH24MI')
...
ORDER BY substr(sdate,1,10)*100+to_char(trunc(to_number(substr(sdate,11,2)/15))*15,'00'),&3 desc--,enb_cell,enb_id

)

where rownum <&5 and &3 > &4  --&6
